      ...
      <div className="flex flex-col lg:flex-row gap-6 items-center justify-between">
        ...
        <div className="flex bg-zinc-900/50 p-1.5 rounded-2xl border border-zinc-800 backdrop-blur-md">
          <button
            ...
            className={`px-6 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all duration-300 flex items-center gap-2 ${activeMode === 'all' ? 'bg-zinc-800 text-white shadow-xl' : 'text-zinc-500 hover:text-zinc-300'
              }`}
          >
            Show All
          </button>
          <button
            ...
            className={`px-6 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all duration-300 flex items-center gap-2 ${activeMode === 'sale' ? 'bg-neon-cyan/10 text-neon-cyan border border-neon-cyan/20' : 'text-zinc-500 hover:text-neon-cyan/70'
              }`}
          >
            <Package size={14} />
            Purchase
          ...
          <button
            ...
            className={`px-6 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all duration-300 flex items-center gap-2 ${activeMode === 'rental' ? 'bg-purple-500/10 text-purple-400 border border-purple-500/20' : 'text-zinc-500 hover:text-purple-400/70'
              }`}
          >
            <Calendar size={14} />
            Rental
      ...
      <div className="flex items-center gap-4 overflow-x-auto pb-4 scrollbar-hide no-scrollbar">
        ...
        {CATEGORIES.map((cat) => (
          <button
            ...
            className={`px-5 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all duration-300 ${activeCategory === cat
